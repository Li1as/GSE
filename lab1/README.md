...
第五阶段详细设计见 [stage5-design.md](stage5-design.md)。5A 已实现经验事件、待审核规则草案、人工批准/拒绝、连续规则版本、停用/恢复以及原子文件发布与中断恢复。5B 已在手机网页接入显式经验提议：用户可以从已纠正的邮件分类或自己保存的草稿版本提出复用要求，由模型生成待审核草案；草案仍须在独立区域编辑并批准后才发布。5C 已将启用规则按 `mail.classify`、`mail.plan`、`mail.draft` 作用域接入新邮件，保存冻结规则快照、逐字依据和应用审计；规则更新不会暗改旧任务，只有显式重新规划才捕获最新版本。5D 已将手机端草案、启用规则和停用规则分区，提供任务专属应用审计接口，并验证新进程复用、停用后新快照排除规则而历史记录仍可查询。5E 已保存可重复邮件工作流，并用隔离端到端测试覆盖完整邮件生命周期、重启去重和第二个合成 ehall 适配器契约。普通纠正和编辑仍不会自动推广，经验规则也不能绕过发送或提交确认。

```bash
# 查看经验事件、草案和已发布规则；完整创建及审核命令见第五阶段设计
python3 lab1/experience.py events
python3 lab1/experience.py proposals
python3 lab1/experience.py rules
python3 lab1/experience.py recover
python3 -m unittest discover -s lab1 -p 'test_experience.py' -v
PYTHONPATH=lab1 python3 -m unittest -v lab1.test_mail_classifier lab1.test_mail_tasks lab1.test_mail_web
...
- 至少保留一条愿意反复使用的跨工具完整流程。更复杂的示例是：收到办事通知 → 检索资料 → 手机补充 → 准备表单和邮件 → 分别确认提交与发送 → 归档。(人工注：ehall 事务暂且保持仅退课，关键之处在于打通了一个 url + 描述 + 附件 到实际业务的链路，以后所有需要多次填写内容/操作的浏览器相关任务都可对其进行扩展适配即可，因此确保当前项目在此方向扩展性即可)
...
- 至少一条跨工具流程可以重复使用，并保留必要的来源、确认和结果证据。(人工注：邮件处理)
